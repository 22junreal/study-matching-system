...
본 시스템은 사용자가 로그인 후 자신의 나이, 성별, 학과, 학번, 선호 스터디 분야, 원하는 학습 요일과 시간, MBTI 정보를 입력하고,  
...
1. 사용자는 회원가입 후 로그인한다.
2. 로그인한 사용자는 자신의 나이, 성별, 학과, 학번, 선호 분야, 요일, 시간대, MBTI 정보를 프로필에 저장한다.
...
graph TD
    A[User Browser] --> B[UI Prototype<br/>HTML / CSS / JavaScript]
